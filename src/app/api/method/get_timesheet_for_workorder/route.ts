...
export async function GET(request: Request) {
  ...
  if (!rrCandidateId) {
    return NextResponse.json({ error: "rr_candidate_id is required." }, { status: 400 });
  }

  const upstreamUrl = new URL(
    `${backendBase}/api/method/te_frappe_6fe.api.timesheet.get_timesheet_for_workorder`
  );
  upstreamUrl.searchParams.set("rr_candidate_id", rrCandidateId);
